...
void Lab8::Update(float deltaTimeSeconds)
{
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, {1, 1, 0});
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, {0.298, 0.298, 0.298});
    ...
    // Render the point light in the scene
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, {0.75, 0.75, 0.75});
    }
}
...
void Lab8::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    glUniform1i(glGetUniformLocation(shader->program, "isASpotlight"), isASpotlight);
    glUniform1f(glGetUniformLocation(shader->program, "cutOffAngle"), cutOffAngle);
    glUniform3fv(glGetUniformLocation(shader->program, "Ke"), 1, glm::value_ptr(Ke));
    glUniform3fv(glGetUniformLocation(shader->program, "Ka"), 1, glm::value_ptr(Ka));
    glUniform3fv(glGetUniformLocation(shader->program, "Kd"), 1, glm::value_ptr(Kd));
    glUniform3fv(glGetUniformLocation(shader->program, "Ks"), 1, glm::value_ptr(Ks));
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_KP_8)) {
            ...
            modelMatrix = glm::rotate(modelMatrix, RADIANS(5.0f), {1, 0, 0});
            lightDirection = modelMatrix * glm::vec4(lightDirection, 1);
        }
        ...
        if (window->KeyHold(GLFW_KEY_KP_4)) {
            ...
            modelMatrix = glm::rotate(modelMatrix, RADIANS(5.0f), {0, 1, 0});
            lightDirection = modelMatrix * glm::vec4(lightDirection, 1);
        }
