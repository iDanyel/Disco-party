...
void Lab9::Update(float deltaTimeSeconds)
{
    ...
    // Why does this happen? How can you solve it?
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, false, false, mapTextures["earth"]);
    }

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, true, false, mapTextures["earth"], mapTextures["crate"]);
    }

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, false, true, mapTextures["random"]);
    }

    {
        ...
        float angle = GetSceneCamera()->m_transform->GetRotationEulerRad().y;

        if (GetSceneCamera()->m_transform->GetLocalOXVector().x < 0)
            angle = glm::pi<float>() - angle;

        modelMatrix = glm::rotate(modelMatrix, angle, glm::vec3(0, 1, 0));
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f));
        RenderSimpleMesh(meshes["square"], shaders["LabShader"], modelMatrix, false, false, mapTextures["grass"]);
    }

    {
        ...
        RenderSimpleMesh(meshes["bamboo"], shaders["LabShader"], modelMatrix, false, false, mapTextures["bamboo"]);
    }
}
...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, bool mixed, bool shouldCycle, Texture2D* texture1, Texture2D* texture2) {
    ...
    glUniform1i(glGetUniformLocation(shader->program, "mixed"), mixed);
    glUniform1i(glGetUniformLocation(shader->program, "shouldCycle"), shouldCycle);
